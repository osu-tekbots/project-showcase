...
class KeywordsActionHandler extends ActionHandler {
    ...
    public function handleMergeKeywords() {
        ...
        $ok = $this->keywordsDao->mergeKeywords($keywordIds);
        if (!$ok) {
    ...
    public function handleUpdateApproval() {
        ...
        $ok = $this->keywordsDao->updateApproval($keywordId, $approved);
        if (!$ok) {
    ...
    public function handleEditKeyword() {
        ...
        $ok = $this->keywordsDao->updateKeyword($keywordId, $keywordText);
        if (!$ok) {
            $this->respond(new Response(Response::INTERNAL_SERVER_ERROR, 'Failed to update keyword'));
        }
    ...
    /**
     * Handles a request to remove a keyword from all relevant showcase projects.
     *
     * @return void
     */
    public function handleRemoveKeyword() {
        ...
        $ok = $this->keywordsDao->removeKeywordEverywhere($keywordId);
        if (!$ok) {
    ...
    public function handleRequest() {
        ...
        $this->logger->error("Handle Request Called with action: ");
